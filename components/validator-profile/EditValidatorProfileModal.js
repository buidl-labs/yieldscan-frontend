...
import { Twitter, Feather, MinusCircle } from "react-feather";
	...
	return (
		<React.Fragment>
			<div className="flex items-center justify-between px-5 py-2 border border-gray-200 rounded-lg">
				...
				{!newMember && <button className="mt-6 ml-2" onClick={() => onRemove()}><MinusCircle color="red" /></button>}
			</div>
			{(newMember && name) && (
...
const EditProfileModal = withSlideIn(
	({ stashId, socialInfo, vision, members, onSuccess, styles, onClose }) => {
