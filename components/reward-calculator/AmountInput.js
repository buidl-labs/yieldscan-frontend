...
const AmountInputDefault = ({ onChange, dollarValue }) => (
...
const AmountInputAlreadyBonded = () => (
	<div className="flex flex-col">
		<div className="flex justify-between bg-gray-200 p-3 rounded-xl">
			<div className="flex flex-col ml-4 rounded-lg py-1">
				...
				<h3 className="text-xl">1500 KSM</h3>
		...
		<div className="bg-gray-800 mt-2 p-3 px-6 flex flex-col rounded-xl">
			...
			<h3 className="text-xl text-white">2000 KSM</h3>
			<span className="text-gray-500 text-xs">$1000</span>
		</div>
	</div>
);

const AmountInput = ({ value, dollarValue, bonded, onChange }) => {
	const currentlyBonded = get(bonded, 'currency', 1);
	return (
		<div className="w-4/5">
			{currentlyBonded ? (
