...
const [useAccounts] = create(set => ({
	...
	setAccountInfoLoading: (accountInfoLoading) => set(() => ({ accountInfoLoading })),
...
const [usePolkadotApi, { getState: getPolkadotApiStoreState }] = create(set => ({
	apiInstance: null,

	setApiInstance: (apiInstance) => set(() => ({ apiInstance })),
}));

export { useAccounts, usePolkadotApi, useTransaction, getPolkadotApiStoreState };
