...
const stake = async (
	...
	if (ledger.isSome) {
		...
		if (stakeAmount > bondedAmount) {
			const amount = (stakeAmount - bondedAmount) * 10 ** networkInfo.decimalPlaces; // 12 decimal places
			transactions.push(api.tx.staking.bondExtra(amount));
		} else {
			const amount = (bondedAmount - stakeAmount) * 10 ** networkInfo.decimalPlaces; // 12 decimal places
