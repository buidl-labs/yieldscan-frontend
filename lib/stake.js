...
const stake = async (
	...
	if (ledger.isSome) {
		...
		if (stakeAmount > bondedAmount) {
			const amount = (stakeAmount - bondedAmount) * 10 ** 12; // 12 decimal places
			console.log("amount");
			console.log(amount);
			transactions.push(api.tx.staking.bondExtra(amount));
		} else {
			const amount = (bondedAmount - stakeAmount) * 10 ** 12; // 12 decimal places
			console.log("amount");
			console.log(amount);
		...
		console.log("Sending nomination request...");
