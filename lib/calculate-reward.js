...
const calculateReward = async (validators = [], amount = 0, timePeriodValue, timePeriodUnit, compounding, bondedAmount) => {
	...
	const returns = compounding ? Number(compoundedReward.toFixed(4)) : Number((totalReward * timePeriodInEras).toFixed(4));
	const portfolioValue = Number((returns + totalAmount).toFixed(4));
